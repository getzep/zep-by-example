...
import {Memory, Message, Session, ZepClient} from "@getzep/zep-js";
...
const ZEP_COLLECTION_DESCRIPTION = "A collection of sci-fi books";

const ZEP_COLLECTION_CONFIG: IZepConfig = {
    apiUrl: ZEP_API_URL,
    apiKey: ZEP_API_KEY,
    collectionName: ZEP_COLLECTION_NAME,
    description: ZEP_COLLECTION_DESCRIPTION,
...
(async () => {
    ...
    // Create a session ID for our conversation. This represents a single
    ...
    const question0 = "What have we discussed so far?";
    const res1 = await conversationChain.run(question0);
